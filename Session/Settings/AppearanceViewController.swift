...
final class AppearanceViewController: BaseVC {
    ...
    private lazy var themeSelectionViews: [ThemeSelectionView] = Theme.availableCases
        .map { theme in
            let result: ThemeSelectionView = ThemeSelectionView(theme: theme) { [weak self] selectedTheme in
                ThemeManager.currentTheme = selectedTheme
                if selectedTheme == .oceanDark || selectedTheme == .oceanLight {
                    ThemeManager.primaryColor = .blue
                } else if selectedTheme == .classicDark || selectedTheme == .classicLight {
                    ThemeManager.primaryColor = .green
                }
            }
